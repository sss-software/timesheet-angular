	...
	function createEditItemForm (btnDomEl) {
		...
		console.log("btnDomEl.dataset.index = " + btnDomEl);
			...
			// CREATE modal-dialog div and contents
			tempFormDoc = document.createElement('div');
			tempFormDoc.setAttribute('class',"modal-dialog");
			tempFormDoc.setAttribute('role',"document");
			tempFormWrap.appendChild(tempFormDoc);

			// CREATE modal-content div and contents
			tempFormContent = document.createElement('div');
			...
			tempFormContent.appendChild(tempFormHeader);
			tempFormHeader.innerHTML = "<h1>EDIT ITEM</h1>";

			tempFormCloseCta = document.createElement('button');
			tempFormCloseCta.setAttribute('class',"close");
			tempFormCloseCta.setAttribute('data-dismiss',"modal");
			tempFormCloseCta.setAttribute('aria-label',"Close");

			tempFormHeader.appendChild(tempFormCloseCta);

			tempFormCloseCtaSpan = document.createElement('span');
			tempFormCloseCtaSpan.setAttribute('aria-hidden',"true");
			tempFormCloseCtaSpan.innerHTML = "x";

			tempFormCloseCta.appendChild(tempFormCloseCtaSpan);

			// CREATE modal-body div
			tempFormBody = document.createElement('div');
			tempFormBody.setAttribute('class',"modal-body");
			tempFormContent.appendChild(tempFormBody);
		  ...
		  // CREATE/OPEN TEMP ITEM EDIT FORM
			...
			tempFormBody.appendChild(tempForm);

			// CREATE modal-footer div
			tempFormFooter = document.createElement('div');
			tempFormFooter.setAttribute('class',"modal-footer");
			tempFormContent.appendChild(tempFormFooter);
	...
	function createJobListFilter(show){
		...
		function addJobItem(date,client,ref,number,jobstart,jobfinish) {
			...
			timesheetObjArray.push(new JobObject(date,client,ref,number,jobstart,jobfinish,false));
